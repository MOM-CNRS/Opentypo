...
/**
 * Colonnes CSV communes aux profils Ceramique, Monnaie et Instrumentum.
 * Les colonnes specifiques sont portees par des classes dediees par profil.
 */
public final class TypologyImportConstants {
    ...
    public static String csvTemplateHeaderOnly() {
        return String.join(",", KNOWN_COLUMNS_COMMON) + "\n";
    }
        ...
        try (InputStream in = TypologyImportConstants.class.getClassLoader().getResourceAsStream(resource)) {
            if (in == null) {
                return switch (profile) {
                    ...
                    case CERAMIQUE, UNSUPPORTED -> String.join(";", TypologyImportCeramiqueConstants.KNOWN_COLUMNS_CERAMIQUE) + "\n";
                };
            }
    ...
    /** Ordre des en-tetes CSV pour les champs communs uniquement. */
    public static final String[] KNOWN_COLUMNS_COMMON = new String[]{
            COL_CODE_CATEGORIE,
            COL_CODE_GROUPE,
            COL_CODE_SERIE,
            COL_CODE_TYPE,
